...
export class DashboardAdmComponent extends MoreComponent implements OnInit, AfterViewInit {
    totalInstitution = 0;
    totalUser = 0;
    totalInstrument = 0;
    totalLocation = 0;
    ...
    ngOnInit(): void {
        this.http.get(`${environment.app.apiUrl}/api/lists/stations/count/supervising-organizations`).subscribe({
            next: data => {
                console.log('options:' + JSON.stringify(data));
                this.totalInstitution = data as number;
        ...
        this.http.get(`${environment.app.apiUrl}/api/lists/stations/count/users`).subscribe({
            next: data => {
                console.log('options:' + JSON.stringify(data));
                this.totalUser = data as number;
                this.spinnerService.hide();
                this.state$.next(true);
            },
            error: err => {
        ...
        this.http.get(`${environment.app.apiUrl}/api/lists/stations/count/instruments`).subscribe({
            next: data => {
                console.log('options:' + JSON.stringify(data));
                this.totalInstrument = data as number;
                this.spinnerService.hide();
                this.state$.next(true);
            },
            error: err => {
        ...
        this.http.get(`${environment.app.apiUrl}/api/lists/stations/count/locations`).subscribe({
            next: data => {
                console.log('options:' + JSON.stringify(data));
                this.totalLocation = data as number;
                this.spinnerService.hide();
                this.state$.next(true);
            },
            error: err => {
        ...
        this.http.get(`${environment.app.apiUrl}/api/lists/top-ten/organizations/locations`).subscribe({
            next: data => {
                ...
                locations.forEach(location => {
                    labels.push(location.short_name ? location.short_name : location.long_name);
                    total.push(location.location_count);
                });
                this.highestLocationData = {
                    labels,
                    datasets: [
                        {
                ...
                //this.highestLocationData = data as number;
                this.spinnerService.hide();
                this.state$.next(true);
            },
            error: err => {
        ...
        this.http.get(`${environment.app.apiUrl}/api/lists/top-ten/organizations/instruments`).subscribe({
            next: data => {
                console.log('instruments:' + JSON.stringify(data));

                const labels: any[] = [];
                const total: any[] = [];
                const instruments = data as any[];
                instruments.forEach(instrument => {
                    labels.push(instrument.short_name ? instrument.short_name : instrument.long_name);
                    total.push(instrument.instrument_count);
                });
                this.highestInstrumentData = {
                    labels,
                    datasets: [
                        {
                            backgroundColor: '#42A5F5',
                ...
                //this.highestInstrumentData = data as number;
                this.spinnerService.hide();
                this.state$.next(true);
            },
            error: err => {
    ...
    ngAfterViewInit(): void {
        ...
        try {
            ...
            this.http.get(`${environment.app.apiUrl}/api/lists/stations/statistics?filter[filled]=false`).subscribe({
                next: response => {
    ...
    changeMarker() {
        ...
        this.stations.forEach(station => {
            ...
            let popup: string;
            //const detail = station.progress_details?.false;
            const detailArr: any[] = station.progress_details;
            let detail = '<ul>';
            detailArr.forEach(detailArr0 => {
                detail += '<li>' + detailArr0 + '</li>';
            });
            detail += '</ul>';

            let colorMarker = 'red';
            if (!this.mapType || this.mapType === 'progress') {
                popup = `<div style="text-align: left; overflow-y: scroll; width: 200px; height: 100px"><b>${
                    station.name
                }</b><br>
                Progress: ${station.progress ? (station.progress * 100).toFixed(2) : 0} % <details>
                    <summary>Fields belum diisi:</summary>${detail}
                  </details><br>
                </div>
                `;

                /*
                popup = `<div style="text-align: left"><b>${station.name}</b><br>
                Progress: ${
                    station.progress ? station.progress * 100 : 0
                } % [<a href="print.html" onclick="window.open('print.html', 
                '', 
                'width=300,height=250,left=50,top=50'); return false;">Detail</a>]<br>
                ${detail}
                </div>
                `;
                */
    ...
    onSelectInstitution(event: any) {
        this.organizationId = event.value;
        const labels: any[] = [];
        const total: any[] = [];

        this.stations.forEach(station => {
            if (station.supervising_organization_id === event.value) {
                labels.push(station.name ? station.name : station.name);
                total.push(station.progress ? (station.progress * 100).toFixed(2) : 0);
            }
        });
    ...
    onSelectProvince(event: any) {
        ...
        this.stations.forEach(station => {
            if (station.province_id === event.value) {
                labels.push(station.name ? station.name : station.name);
                total.push(station.progress ? (station.progress * 100).toFixed(2) : 0);
            }
        });
