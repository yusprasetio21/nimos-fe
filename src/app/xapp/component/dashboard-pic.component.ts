...
@Component({
    ...
    templateUrl: './dashboard-pic.component.html'
})
export class DashboardPicComponent extends MoreComponent implements OnInit, AfterViewInit {
    totalInstitution = 0;
    totalUser = 0;
    totalInstrument = 0;
    totalLocation = 0;
        ...
        try {
            ...
            this.http.get(`${environment.app.apiUrl}/api/lists/stations/statistics?filter[filled]=false`).subscribe({
                next: response => {
                    ...
                    this.totalLocation = this.stations.length;
                    this.changeMarker();
                },
                error: err => {
                    this._errorHandler(err);
    ...
    changeMarker() {
        ...
        this.stations.forEach(station => {
            ...
            if (!this.mapType || this.mapType === 'progress') {
                popup = `<div style="text-align: left; overflow-y: scroll; width: 200px; height: 100px">
