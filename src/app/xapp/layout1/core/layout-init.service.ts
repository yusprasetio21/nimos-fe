...
export class LayoutInitService {
    ...
    private initLayout() {
        ...
        const selfBodyBackgroundImage = this.layout.getProp('main.body.backgroundImage');
        if (selfBodyBackgroundImage) {
            document.body.style.backgroundImage = `url("${selfBodyBackgroundImage}")`;
        }

        const selfBodyClass = (this.layout.getProp('main.body.class') || '').toString();
        if (selfBodyClass) {
    ...
    private initHeader() {
        this.layout.setCSSClass(
            'headerContainer',
            this.layout.getProp('header.width') === 'fluid' ? 'container-fluid' : 'container-xxl'
        );
        ...
        const tabletAndMobile = this.layout.getProp('header.fixed.tabletAndMobile') as boolean;
        if (tabletAndMobile) {
    ...
    private initPageTitle() {
        ...
        if (responsive) {
            ...
            const responsiveBreakpoint = this.layout.getProp('pageTitle.responsiveBreakpoint') as string;
            const responsiveTarget = this.layout.getProp('pageTitle.responsiveTarget') as string;
    ...
    private initToolbar() {
        ...
        this.layout.setCSSClass('toolbarContainer', widthClass === 'fluid' ? 'container-fluid' : 'container-xxl');

        const fixedDesktop = this.layout.getProp('toolbar.fixed.desktop') as boolean;
        if (fixedDesktop) {
            document.body.classList.add('toolbar-fixed');
        }

        const fixedTabletAndMobileMode = this.layout.getProp('toolbar.fixed.tabletAndMobileMode') as boolean;
    ...
    private initContent() {
        ...
        this.layout.setCSSClass('contentContainer', width === 'fluid' ? 'container-fluid' : 'container-xxl');
    }
    ...
    private initFooter() {
        ...
        this.layout.setCSSClass('footerContainer', width === 'fluid' ? 'container-fluid' : 'container-xxl');
    }
}
